    ...
    def self.universal_enhanced_code(options = {})
      ...
      if options[:transaction]
        ...
        options[:products_in_order].each do |product_hash|
          ecommerce_code << "ga('ecommerce:addProduct', #{product_hash.to_json});\n"
        end
        ...
        # });
        ecommerce_code << "a('ec:setAction', 'purchase', #{options[:transaction].to_json});\n"
        ...
        #       'price': '29.20',
        #     });
        ecommerce_code << "ga('ecommerce:addProduct', #{options[:product_detail_options].to_json});\n"
        ecommerce_code << "a('ec:setAction', 'detail'});\n"
      end
